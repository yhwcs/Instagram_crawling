...
insta_id = input("인스타그램 아이디를 입력하세요 : ")
insta_pw = input("인스타그램 비밀번호를 입력하세요 : ")
...
# 미통과 미미 수준 'istp','infp','esfp'** 10개 정도로 수정? 무슨 이상한 지역이 있음ㅋㅋㅋ
# 미통과 심각 수준 'estj','estp'**굉장히 문제가 많음  est들은 인스타를 안하는 건가
# 통과 'istj','isfj','isfp','intj','intp','infj', 'esfj','entj','entp','enfj','enfp'
search_name = ['istj', 'isfj', 'isfp', 'intj', 'intp', 'infj', 'esfj', 'entj', 'entp', 'enfj', 'enfp']
    ...
    for i in range(len(search_id)):
        ...
        if mbti not in search_id[i].text:
            ...
            if cnt == max or i == len(search_id) - 1:
                ...
                search_id = driver.find_elements_by_css_selector("div._7UhW9.xLCgt.qyrsm.KV-D4.uL8Hv")
            # if cnt == max:
            #   break # print(f"mbti {search_name}의 계정을 총 {max}개 찾았습니다")
